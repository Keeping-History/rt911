...
- `src/Applications/*` — one folder per product app (Browser, News, TV, RadioTraffic, RadioTuner, PagerDecoder, Newsgroups, TimeMachine, Feedback, Account), each a `ClassicyApp` + `ClassicyWindow(s)`. `src/Applications/radio-core/` is not an app — it's the shared library (station grouping/audio pipeline/players/settings) that RadioTraffic, RadioTuner, PlaylistEditor, MarketWatch, Mobile and `Providers/Playlist` all build on; the dependency arrow points one way (apps depend on radio-core, never the reverse — enforced by `radio-core/noAppImports.test.ts`). It's what's left after the old RadioScanner app was split into RadioTraffic (comm/ATC traffic) and RadioTuner (continuous broadcast stations) with their shared code lifted out first.
- `src/Providers/MediaStream/` — the single WebSocket client to the `packages/backend` streamer.
- `src/Providers/Playlist/` — the teacher-playlist engine (spec: `plans/2026-07-16-teacher-playlists-design.md`): loads `?playlist=<id>` from Directus and enforces availability windows / app disabling / focus / scheduled events. Its runtime state is deliberately non-persisted (no ClassicyStore, no localStorage, no ClassicyFileSystem). `RoomControlBridge.tsx` sits alongside it and applies a teacher's **live** commands (jump/focus/message/lock), which the streamer pushes over the WebSocket to everyone following the same `?playlist=` id — see "Room control" in the backend's `docs/websocket-protocol.md`. The teacher end is a `Control` menu on each playlist document window in the Playlists app (`Applications/PlaylistEditor/playlistMenus.ts`'s `documentControlMenu`, wired up in `PlaylistDocumentWindow.tsx`), which POSTs to the streamer's `/room` via `roomApi.ts`; the streamer authorises those per playlist, so only the playlist's creator can drive it.
...
5. **`classicy` is external, pinned to an exact version; don't hand-edit it.** `.husky/pre-commit` auto-bumps it on every commit (`pnpm update classicy --latest --recursive`, which rewrites both `pnpm-lock.yaml` and this package's version pin — stage both, not just the lockfile) — see the root `CLAUDE.md`. Use `pnpm use:local` / `pnpm use:published` to develop against an unpublished local build.
