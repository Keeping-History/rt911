...
- **One virtual clock, two writers.** The canonical time lives in Classicy's own state at `state.System.Manager.DateAndTime` (`{ dateTime, timeZoneOffset }`), seeded in `app.tsx` to `2001-09-11T12:40:00.000Z` / `-4`. **`TimeMachine.tsx` (user seeks) and the Playlist engine (`src/Providers/Playlist/PlaylistProvider.tsx`, scheduled `jump` entries) are the only writers, and both go through `setDateTimeFromUtc` in `TimeMachine/setVirtualClock.ts`.** Every other app — `TV`, `RadioScanner`, `News`, `PagerDecoder`, `MediaStreamProvider` — reads it reactively via `useClassicyDateTime({ tick: true })` (when it needs play/pause/`localDate`) or `useAppManager((s) => s.System.Manager.DateAndTime.dateTime)` (when it just needs the value). Don't add a second place that calls `setDateTime`/`shiftTime`.
...
2. **Only `TimeMachine.tsx` (desktop), the `src/Mobile/screens/` Time Travel screens (mobile — the two are never mounted together), and the Playlist engine (`src/Providers/Playlist/PlaylistProvider.tsx`, scheduled `jump` entries) mutate the clock — all via `setDateTimeFromUtc` in `TimeMachine/setVirtualClock.ts`.** A new app that needs to "jump to a time" should call into that same helper, not add its own `setDateTime` call — every other app assumes the clock only moves from those places.
