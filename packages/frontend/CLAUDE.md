...
2. **All clock writes go through `setDateTimeFromUtc` in `TimeMachine/setVirtualClock.ts` — no exceptions.** The sanctioned callers are `TimeMachine.tsx` (desktop) and the `src/Mobile/screens/` Time Travel screens (mobile — the two are never mounted together) for user-driven seeks; the Playlist engine (`src/Providers/Playlist/PlaylistProvider.tsx`, scheduled `jump` entries) for playlist jumps; and `MediaStreamProvider` for the streamer's forced clock mode (server-pushed `clock`/`heartbeat_ack.master_time` frames — see the backend's `docs/websocket-protocol.md`). A new app that needs to "jump to a time" should call into that same helper, not add its own `setDateTime` call — every other app assumes the clock only moves from those places, and while forced mode is active `DateAndTime.dateTimeLocked` is set so user-driven writers (TimeMachine, mobile Time Travel) can't fight the master.
