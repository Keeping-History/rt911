...
describe("FeedbackForm", () => {
	...
	it("calls onSubmit with form fields and empty attachments on submit", async () => {
		...
		fireEvent.mouseUp(screen.getByRole("button", { name: /send feedback/i }));
	...
	it("removes a file when its remove button is clicked", async () => {
		...
		fireEvent.mouseUp(screen.getByRole("button", { name: /remove photo\.png/i }));
	...
	it("calls onCaptureScreenshot and prepends the result to attachments", async () => {
		...
		fireEvent.mouseUp(screen.getByRole("button", { name: /capture screenshot/i }));
