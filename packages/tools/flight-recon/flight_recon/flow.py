...
$DIRECTUS_API_TOKEN must be in the environment (injected from the
flight-recon-secrets Kubernetes Secret — never a parameter, never logged).
"""
...
@task(**NETWORK_RETRIES)
def load_positions(positions, run_id, start, end, directus_url):
    log = get_run_logger()
    client = _client(directus_url)
    try:
...
def load_tracks(tracks, run_id, start, end, directus_url):
    ...
    try:
        ...
        inserted = client.insert_many("flight_tracks", rows)
...
def reconstruct_flights(
    ...
    log.info("run_id=%s window=[%s, %s]", run_id, start, end)
    ...
    n_pos = load_positions(positions, run_id, start, end, directus_url)
