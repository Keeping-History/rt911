...
CI (`.github/workflows/build.yml`) runs `tsc -b`, `oxlint .`, and `vitest run` for the frontend — each as its own parallel job (`frontend-typecheck`, `frontend-lint`, `frontend-test`) — and `go test ./...` for the backend, all as required checks before building/pushing images. Playwright E2E is also required: it runs in parallel with the three frontend-* jobs, and the `frontend` image job `needs:` all four, so a red E2E (or a red typecheck/lint/test) blocks both the merge and the GHCR push that Argo CD rolls out.
