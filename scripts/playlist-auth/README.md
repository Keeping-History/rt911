...
- Sign in with Apple configured (2026-07-16): Services ID org.911realtime.auth, openid
  driver, full discovery URL, scope "name email", response_mode=form_post (infra commit
  ebc57a1). Client secret is an ES256 JWT generated by generate-apple-secret.sh —
  **expires 2027-01-12; rotate before then** (regenerate + patch rt911-secrets
  AUTH_APPLE_CLIENT_SECRET + rollout restart). /auth lists google + apple.

## Apple client-secret rotation

```sh
set -a; source ~/auth-secrets.env; set +a
JWT=$(./generate-apple-secret.sh ~/AuthKey_${APPLE_KEY_ID}.p8)
kubectl patch secret rt911-secrets -n rt911 --type merge \
